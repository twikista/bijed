...
async function Article({ params }) {
  ...
  const data = await Promise.all([
    getArticle(params),
    JobQueue.find({ jobRef: `${params.issue}` }),
  ])
  const [article, [jobRef]] = data

  console.log('para----- ', article)
  console.log('data---', jobRef)
        ...
        <div>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex gap-6 mb-3'>
              <EditButton
                href={`/dashboard/issues/unpublished/${params.issue}/${params.article}/edit`}
                label='edit'
                disabled={jobRef?.status === ('pending' || 'approved')}
              />
              <DeleteButton
                variant='primary'
